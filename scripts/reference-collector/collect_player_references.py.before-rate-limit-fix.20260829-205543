...
# Process-wide pacing prevents concurrent workers from overrunning Wikimedia
# and Brave. Collection is serialized below for deterministic face matching.
...
WIKIMEDIA_MIN_INTERVAL = float(os.getenv("ERACLASH_WIKIMEDIA_DELAY_SECONDS", "1.35"))
BRAVE_MIN_INTERVAL = float(os.getenv("ERACLASH_BRAVE_DELAY_SECONDS", "1.20"))
DOWNLOAD_MIN_INTERVAL = float(os.getenv("ERACLASH_DOWNLOAD_DELAY_SECONDS", "0.20"))
MAX_API_RETRIES = int(os.getenv("ERACLASH_API_RETRIES", "7"))


def wait_for_request_slot(provider: str, interval: float) -> None:
    with _RATE_LOCK:
        now = time.monotonic()
        ready_at = _NEXT_REQUEST_AT.get(provider, 0.0)
        wait = max(0.0, ready_at - now)
        _NEXT_REQUEST_AT[provider] = max(now, ready_at) + max(0.0, interval)
    if wait > 0:
        time.sleep(wait)
...
def retry_delay(response: requests.Response | None, attempt: int) -> float:
    if response is not None:
        ...
        try:
            seconds = float(value)
            if seconds > 0:
                return min(120.0, seconds)
        except ValueError:
            pass
    return min(90.0, 2.5 * (2 ** attempt))
...
def request_with_retry(
    ...
    **kwargs: Any,
    ...
    for attempt in range(MAX_API_RETRIES):
        wait_for_request_slot(provider, interval)
        ...
        try:
            ...
            if response.status_code in retryable:
                ...
                print(
                    f"[{provider}] HTTP {response.status_code}; retrying in "
                    f"{delay:.1f}s ({attempt + 1}/{MAX_API_RETRIES})",
                    file=sys.stderr,
                )
                response.close()
                time.sleep(delay)
            ...
            if response.status_code >= 400:
                try:
                    ...
                    body = ""
                raise ApiError(
                    f"{provider} HTTP {response.status_code} for {response.url}"
            ...
            delay = retry_delay(response, attempt)
            print(
                f"[{provider}] request failed: {exc}; retrying in "
                f"{delay:.1f}s ({attempt + 1}/{MAX_API_RETRIES})",
                file=sys.stderr,
            )
            time.sleep(delay)
    ...
    raise ApiError(
        f"{provider} request failed after {MAX_API_RETRIES} attempts: {last_error}"
    )
...
def wikidata_entity(name: str) -> dict[str, Any] | None:
    ...
    response = request_with_retry(
        ...
        provider="wikimedia",
        interval=WIKIMEDIA_MIN_INTERVAL,
        params=params,
...
def brave_search(query: str, api_key: str, count: int = 20, era_hint: str = "") -> list[Candidate]:
    headers = {
    ...
    for row in rows:
        ...
        source_value = row.get("source") or row.get("page_url") or row.get("source_page_url") or ""
        if isinstance(source_value, dict):
            source_page = str(source_value.get("url") or source_value.get("page_url") or "")
        else:
            source_page = str(source_value)
    ...
    for url in urls:
        try:
            with request_with_retry(
                ...
                url,
                provider="download",
                interval=DOWNLOAD_MIN_INTERVAL,
                timeout=REQUEST_TIMEOUT,
                stream=True,
...
def collect_person(
    ...
    if resume and not force and manifest_path.exists():
        ...
        selected_count = len(prior.get("selectedIdentity", []))
        downloaded_count = len(prior.get("candidates", []))
        if (
            prior.get("status") == "AUTO_SELECTED"
            and selected_count >= target
            and downloaded_count >= target
        ):
...
def main() -> int:
    ...
    if args.workers != 1:
        print(
            f"NOTICE: Requested {args.workers} workers; using 1 shared-rate-limited "
            "worker for reliable Wikimedia/Brave collection.",
            file=sys.stderr,
        )
