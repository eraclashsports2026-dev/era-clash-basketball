...
const journey = async (ctx, name, mobile, { full = true, phone = mobile } = {}) => {
  ...
  const wrap = await p.evaluate(() => { const r = document.createRange(); const bad = []; for (const n of document.querySelectorAll(".ec-pc-name, .ec-coach-name")) { const t = n.firstChild; if (!t || t.nodeType !== 3) continue; const text = t.textContent; let pos = 0; for (const w of text.trim().split(/[\s-]+/)) { if (!w) continue; const i = text.indexOf(w, pos); if (i < 0) continue; pos = i + w.length; r.setStart(t, i); r.setEnd(t, i + w.length); if (r.getClientRects().length > 1) bad.push(`${w} split`); } const k = n.querySelector(".ec-pc-kept"); if (k && k.getBoundingClientRect().height > 20) bad.push("KEPT split"); } return bad; });
