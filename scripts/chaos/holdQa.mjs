...
import { startRun, submitRollDecisions, publicView } from "../../src/chaos/runState.js";
...
for (const n of [0, 1, 2, 3, 4, 5]) {
  ...
  const res = submitRollDecisions(r, { holdSlots: slots, holdRoles: [], hydrate });
...
// Malformed submissions are refused rather than coerced.
{
  ...
  ok("a duplicate slot is refused", submitRollDecisions(r, { holdSlots: ["PG", "PG"], holdRoles: [], hydrate }).code === "DUPLICATE_SLOT");
  ok("an unknown slot is refused", submitRollDecisions(r, { holdSlots: ["ZZ"], holdRoles: [], hydrate }).code === "UNKNOWN_SLOT");
  ok("a non-array is refused", submitRollDecisions(r, { holdSlots: "PG", holdRoles: [], hydrate }).code === "VALIDATION_FAILURE");
...
// Roll 2: kept cards come back pre-selected, and can be released.
{
  ...
  submitRollDecisions(r, { holdSlots: ["SG"], holdRoles: [], hydrate });
  ...
  ok("a fourth roll is refused", submitRollDecisions(r, { holdSlots: [], holdRoles: [], hydrate }).ok === false);
...
// Burned cards never return; one person never appears twice.
{
  ...
  submitRollDecisions(r, { holdSlots: [], holdRoles: [], hydrate });
  submitRollDecisions(r, { holdSlots: [], holdRoles: [], hydrate });
