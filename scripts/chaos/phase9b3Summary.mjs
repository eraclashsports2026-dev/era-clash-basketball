...
const ledger = [
  ...
  { item: "deployed QA on the durable alias", state: st(secret?.passed && DEPLOYED), evidence: secret ? `secret-audit.json — ${secret.bundle?.secretShaped + secret.bundle?.serviceRoleJwt} secret-shaped strings` : "not run" },
