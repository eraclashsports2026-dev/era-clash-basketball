...
const ledger = [
  ...
  { item: "phone: the primary action follows its decision in flow and covers nothing (was: sticky at the top of every decision state)", state: st(["foundation-qa", "drafting-qa", "era-reveal-qa", "coach-chaos-qa", "clash-ready-qa"].every((n) => (stateQa[n]?.runs || []).filter((r) => /^(430|390)x/.test(r.viewport)).every((r) => r.facts?.ctaOnScreen === true))), evidence: "state QA facts.ctaOnScreen at 430×932 and 390×844 — sticky action engaged (overflow: clip on the stage and court), head labels hidden on phones" },
