#!/usr/bin/env bash
# Phase 9E gate sweep — serial, one log. Heavy gates never overlap.
#   bash scripts/competitive/phase9eSweep.sh [deployedOrigin]
...
LOG=data/validation/9e/${SWEEP_LOG:-final-gate-sweep.log}
...
git checkout -- data/validation/7a data/validation/7b data/validation/8c-time-arena data/validation/8c1 data/validation/9a data/validation/9a1 data/validation/9a3p data/validation/9a2 data/validation/9a3 data/validation/9b1 data/validation/9b1a data/validation/9b2 data/validation/9b3 data/validation/9c data/validation/9d 2>/dev/null || true
git status --short data/validation | grep -v '9e/' | head
