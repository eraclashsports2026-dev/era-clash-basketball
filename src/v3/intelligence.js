...
export const ROLE_DEFINITIONS = [
  ...
  { role: "Secondary Creator",       about: "Can create a shot when the first option is taken away, without needing the ball to stay useful.",    requires: ["selfCreation", "roleAcceptance"],          fit: (o, d, f) => lo(o.selfCreation, f.roleAcceptance) * 0.65 + o.passingVision * 0.35 },
  ...
  { role: "Spot-Up Spacer",          about: "Punishes help by standing still and making the pass-out shot. Costs nothing to play beside.",         requires: ["spacingGravity", "roleAcceptance"],        fit: (o, d, f) => lo(o.spacingGravity, f.roleAcceptance) * 0.70 + o.offBallMovement * 0.30 },
  ...
  { role: "Roll Threat",             about: "Scores off someone else's advantage — rolls, cuts, dives, and finishes what the defence concedes.",   requires: ["rimThreat", "offBallMovement"],            fit: (o, d, f) => lo(o.rimThreat, o.offBallMovement) * 0.65 + f.roleAcceptance * 0.35 },
  ...
  { role: "Connector",               about: "Keeps the ball moving and the spacing honest. Adds value without consuming any.",                     requires: ["connectivity", "roleAcceptance"],          fit: (o, d, f) => lo(f.connectivity, f.roleAcceptance) * 0.65 + o.passingVision * 0.35 },
  { role: "Low-Usage Finisher",      about: "Converts what the offence hands them and asks for nothing back.",                                     requires: ["roleAcceptance", "rimThreat"],             fit: (o, d, f) => lo(f.roleAcceptance, o.rimThreat) * 0.60 + (10 - o.usageAppetite) * 0.40 },
];
...
export const ATTRIBUTE_DEFINITIONS = {
  ...
  // fit
  roleAcceptance:       { block: "fit", means: "Value retained at MINIMUM touches. High means they are still worth a starting spot on nothing; low means they need the ball to be themselves. Not a character judgement — a skill-portability measurement." },
  ...
  creationDependence:   { block: "fit", means: "How much of their value evaporates when someone else runs the offence. The inverse pressure to roleAcceptance." },
...
export const buildIntelligence = (p, ctx = {}) => {
  ...
  const offense = {
    ...
    spacingGravity: r1(dna.outsideShooting * 0.7 + dna.threeTendency * 0.3),
  ...
  const fit = {
    roleAcceptance: r1(dna.offBall * 0.5 + (10 - dna.ballDominance) * 0.28 + (10 - dna.usageTendency) * 0.14 + dna.iq * 0.08),
  ...
  const profile = {
    ...
    // NEVER invented. The trusted dataset carries no measurements; a plausible
    // height is still a fabricated one, and it would read like a record.
    physical: { heightIn: null, weightLb: null, wingspanIn: null },
  ...
  profile.provenance = {
    ...
    physical: "ABSENT — the dataset holds no verified heights, weights, or wingspans. These stay null rather than being estimated from position.",
...
export const validateIntelligence = (profile) => {
  ...
  // physical must stay unmeasured — a number here means someone invented one
  if (!profile.physical || typeof profile.physical !== "object") e("physical block missing");
  else for (const k of ["heightIn", "weightLb", "wingspanIn"]) {
    if (!(k in profile.physical)) e(`physical.${k} missing`);
    else if (profile.physical[k] !== null) e(`physical.${k} must be null — no verified measurement source exists`);
  }
  ...
  else for (const k of ["offense", "defense", "roles", "physical", "overall"]) if (!cf[k]) e(`confidence.${k} missing`);
