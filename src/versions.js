...
export const REGISTRY = {
  ...
  possessionCalibrationVersion: entry("1.4.0", DEVELOPMENT_LOCKED_SCOPED,
    "Candidate 2: the identity-transfer repair of Candidate 1 (parent 1.1.0, which stays LOCKED at its recorded hashes and replays from its preservation commit; grandparent 1.0.0 likewise). Candidate 1 failed Historical Holdout V5 on one frozen gate, zeroTraitHardFails, with two independent measurement failures. Both root-caused to the same class of defect Candidate 1 itself was built to fix: an identity signal that reaches part of the engine and then stops. Two engine changes, both centred so a neutral coach is an exact fixed point. First, the offence's documented ball-movement identity now scales the finishing of a pass-created look into a credited assist, restoring a lever the possession rewrite dropped from src/v3/possession.js. Second, coach scheme intent transfers partially past the personnel ceiling instead of being truncated to it, and the resulting coaching differential reaches opponent shot quality. All 53 active parameters remain at registry defaults. Locked in Phase 6C4C1, scoped to a future Historical Holdout V6. NOT holdout validated, NOT preview validated, NOT production ready. 1.3.0 (Candidate 3, parent 1.2.0 which stays LOCKED at its recorded hashes): coach offensive-action intent now carries partially past the roster-capability gate in the isolation, off-ball-screen and cut family weights (INTENT_CARRY, floor 0.5) — the offensive twin of Candidate 2's defensive SCHEME_TRANSFER. Frequency only; execution quality is still priced at resolution, and a neutral coach remains an exact mirror fixed point. Motivated by a defect proven on non-V6 mirror controls where a motion-10 coach with sub-threshold movers expressed zero movement-share difference from a neutral coach. Also 1.3.0: the calibration adapter's postThreat derivation now weights interior scoring volume over rebounds — under the old rule nine sub-10-ppg corpus players rated 7+ and Ben Wallace out-threatened Yao Ming, so post-up baselines were set by players who cannot score on the block. 1.4.0 (Candidate 4, parent 1.3.0 which stays LOCKED at its recorded hashes): four defects found by the whole-project code audit of 2026-08-31, none of them a parameter change and all 53 still at registry defaults. (1) In a pre-three-point era the three-point weight was zeroed by the era volume scale BEFORE the documented transfer to MIDRANGE, so \'the shot goes, the skill does not\' moved exactly nothing in the 1950s, 1960s and 1970s and the perimeter weight was renormalised away instead. (2) ZONE_ATTACK could be drawn on a man possession, where resolveAction has no route for it and it fell through to the generic resolver — and the ATTACK_ZONE_* coach adjustments bump exactly that family into exactly that mix, so the one control that raised it could only raise it where it could not work. (3) A pulled-out transition possession bypassed the opportunity allocator; the obvious fix of passing alloc through INVERTED the usage hierarchy outright, because the transition draw had already consumed an attempt for a player who never shot, so the possession now settles whether it is still a break before drawing anyone. (4) The POST_UP kickout applied the receiver\'s matchup shot-quality modifier twice via `- x * -1 + x`, one-sidedly, the upper clamp having eaten the upside. Locked in Phase 8D. Inherits Candidate 3\'s formal standing unchanged: NOT holdout validated, NOT preview validated, NOT production ready, and no holdout was opened for it.", false),
  ...
  actionLibraryVersion: entry("2.1.0", DEVELOPMENT, "src/v3/actions/ — versioned basketball action models. Pick-and-roll is the first. 2.1.0 (Candidate 3): INTENT_CARRY in the isolation, off-ball-screen and cut family weights — the coach term carries past the roster reach() gate at half strength minimum; roster, era and mismatch terms remain gated.", false),
